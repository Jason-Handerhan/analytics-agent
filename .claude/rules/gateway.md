...
- Enforcing the ~90s timeout via `asyncio.wait_for(...)` around the
  **`astream`** consumption below — not `ainvoke`. On
  timeout return a normal `AgentResponse`, not an HTTP error — Power Apps needs
  no new UI code. Cancellation propagation details are in the orchestrator rule.
...
object that's structurally impossible.
