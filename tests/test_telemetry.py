...
async def test_write_telemetry_row_success(monkeypatch):
    ...
    # typed for BigQuery (STRING columns JSON-dumped, datetimes isoformat'd),
    # not left null.
    assert row["conversation_id"] == "conv-123"
    assert row["turn_started_at"] == STARTED.isoformat()
    assert row["turn_completed_at"] == COMPLETED.isoformat()
    ...
    assert row["tool_calls"][0]["started_at"] == STARTED.isoformat()

    # errors: tool_call_id is the join key back to tool_calls.id
    assert row["errors"][0]["tool_call_id"] == "tc1"
    assert row["errors"][0]["occurred_at"] == STARTED.isoformat()
